...
| `map(list, lambda) -> list` | 对列表每个元素应用 lambda | xan/jq |
| `filter(list, lambda) -> list` | 过滤列表元素 | xan/jq |
| `reduce(list, init, op) -> val` | 列表归约 | xan/jq |

**Lambda 表达式**:
- 语法: `x => expr` 或 `(x, y) => expr` 或 `() => expr`
- 示例: `map([1, 2, 3], x => x * 2)` 返回 `[2, 4, 6]`
- 示例: `filter([1, 2, 3, 4], x => x > 2)` 返回 `[3, 4]`
- 示例: `reduce([1, 2, 3], 0, (acc, x) => acc + x)` 返回 `6` |
...
Lambda 表达式用于创建匿名函数，主要用于高阶函数（如 `map`, `filter`, `reduce`）：
...
│   ├── string.rs       # 字符串函数
│   ├── numeric.rs      # 数值函数
│   ├── list.rs         # 列表函数（含 map/filter/reduce）
│   ├── logical.rs      # 逻辑控制函数
│   ├── regex.rs        # 正则表达式函数
│   ├── hash.rs         # 编码哈希函数
│   ├── datetime.rs     # 日期时间函数
...
pub struct EvalContext<'a> {
    ...
    pub variables: HashMap<String, Value>,      // 行级变量存储（as 绑定）
    pub lambda_params: HashMap<String, Value>,  // Lambda 参数绑定
}
```
...
pub enum Expr {
    ...
    // 列引用
    ColumnRef(ColumnRef),  // @1, @col_name
    
    // 变量
    Variable(String),      // as 绑定的变量
    
    // Lambda 表达式
    Lambda { params: Vec<String>, body: Box<Expr> },  // x => x + 1
    LambdaParam(String),   // Lambda 参数引用
    ...
    MethodCall { object: Box<Expr>, name: String, args: Vec<Expr> },
    Pipe { left: Box<Expr>, right: PipeRight },
}
...
## 2. 现有方案分析 (Landscape Analysis)

### GNU Parallel: 模板替换 (Template Substitution)
...
### AWK: 记录与字段处理模型 (Record and Field Processing Model)
...
### xan (Moonblade): 动态表达式引擎 (Dynamic Expression Engine)
...
### jq: JSON 处理器与流式计算 (JSON Processor & Stream Computing)
...
### Tera: 模板引擎与 Rust 生态 (Template Engine & Rust Ecosystem)
