...
- 第二阶段（进行中）：逐步覆盖 `tsv-append`、`tsv-split`、`tsv-sample`、`tsv-uniq`、`tsv-select` 等中等复杂度工具；其中 `tsv-append` / `tsv-sample` / `tsv-select` 已分别在 `tva` 中落地为 `append` / `sample` / `select` 子命令，并配套 golden 测试，`tsv-split` / `tsv-uniq` 仍在规划中。
