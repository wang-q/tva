...
### 2.3 `tsv-utils` → `tva` 测试迁移进度

目前各工具的迁移情况（以上游各自的测试资源为基准）：

- `number-lines` / `tva nl`（以上游 `gold/basic_tests_1.txt` 为基准）：
  - 上游 golden 行为块数量：20（`====[number-lines ...]====`）
  - `tva` 本地 `nl` 子命令 CLI 测试数量：27（`tests/cli_nl.rs` 中的 `#[test]`）
    - 其中 20 个测试直接对齐上述 golden 行为块
    - 额外增加 7 个测试覆盖 `--help`、`--version` 以及典型错误场景（不存在的输入文件、未知选项等）
