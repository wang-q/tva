...
6. **公共基础设施（`common/` 目录）**
   ...
   - 从迁移优先级上看，可以粗略分为三批：
    - **第一批（基础设施）**：字段列表解析、输入源抽象——这两者对后续大多数子命令都有直接帮助，适合在更多工具落地前先设计好 Rust 版实现。当前进度与后续计划：
       - 字段列表解析：已在 `libs::fields` 模块中实现数字区间语法（如 `1,3-5`），通过 `parse_numeric_field_list` / `fields_to_ints` / `fields_to_idx` 为 `dedup`、`md` 等子命令提供统一的数字字段列表解析；后续会在此基础上扩展到按字段名和通配符字段名的语法，并抽象出公共字段列表接口，供未来的 `tva sort`、`tsv-select`、`tsv-uniq` 等共享。
       - 输入源抽象：已在 `libs::io` 模块中实现统一的输入层：`reader` 负责处理 `stdin` / `-` / 普通文件 / `.gz` 压缩文件，`InputSource` / `input_sources` 为多输入文件提供一致的视图，`has_nonempty_line` 封装了“探测是否包含非空行”的逻辑；目前 `nl`、`dedup`、`keep-header` 等子命令已经复用这一输入基础设施，后续 `tva sort` 也会直接建立在该抽象之上，并在适当时机统一错误信息格式与多文件遍历策略。
...
在 `tva` 项目中，可以把 tsv-utils 视为一个“功能地图”和“经验库”：一方面按需对标其工具集合规划 `tva` 的子命令，另一方面在实现与文档风格上保持一致或有意识地作出改进；与此同时，`common/` 目录提供的抽象也可以视为未来在 Rust 中设计公共库（字段语法、输入源、数值工具等）的重要灵感来源。
