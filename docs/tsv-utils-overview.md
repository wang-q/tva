...
6. **公共基础设施（`common/` 目录）**
   ...
   - 从迁移优先级上看，可以粗略分为三批：
     - **第一批（基础设施）**：字段列表解析、输入源抽象——这两者对后续大多数子命令都有直接帮助，适合在更多工具落地前先设计好 Rust 版实现。具体计划：
       - 字段列表解析：在现有 `libs::fields_to_idx` 等函数基础上，抽象出一套统一的“字段列表语法”模块，首先覆盖 `1,3-5` 这类数字区间语法，后续扩展到字段名和通配符字段名，并让 `tva sort`、未来的 `tsv-select`、`tsv-uniq` 等共享这一实现。
       - 输入源抽象：在 `libs::reader` 的基础上增加“多输入源迭代器”，统一处理 `stdin`、多文件、压缩输入以及错误信息格式，为 `nl`、`keep-header`、`sort` 等子命令提供一致的输入视图。
