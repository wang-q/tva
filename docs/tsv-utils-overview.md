...
5. **排序工具与 `tva sort` 规划**
   - 上游 tsv-utils 本身没有提供独立的 `sort` 可执行程序，而是通过 `keep-header` 搭配外部 `sort`，以及 `extras/scripts/tsv-sort-fast` 这类 Bash 脚本来完成排序相关场景。这在 Unix 环境下配合 GNU coreutils 很自然，但：
     - 在 Windows 等平台上，对外部 `sort` 的依赖会引入编码、行尾等差异；
     - 对于希望“一站式”使用 Rust 工具链的场景来说，缺少一个原生的、表格感知的排序命令略显遗憾。
   - `tva` 中后续会增加一个 `sort` 子命令，作为“tsv-aware 排序工具”，整体思路参考三类现有实现：
     - `tsv-sort-fast`：保留“流式 + 外部归并”的大文件友好策略，优先支持按字段（列）排序，而不是整行字符串排序；
     - `rsv-0.4.20.1`：借鉴其在 Rust 中处理表格数据流、选项解析和错误管理的方式，尽量让 `tva sort` 直接复用现有的 reader/record 抽象，而不是重新发明一套 IO 层；
     - `qsv-16.1.0` 中的 `sort`：参考其列选择语法、多 key 排序、数值/字符串模式、升降序控制以及大文件外部排序/归并策略，在保证实现简洁的前提下，从一开始就考虑到实践中的性能需求。
   - 第一阶段目标（MVP）：
     - 子命令名：`tva sort`
     - 面向 TSV/CSV，默认按 Tab 分隔，可通过选项切换分隔符；
     - 支持单 key、多 key 排序，key 可以按列号指定；在内部数据结构允许的前提下，预留按列名排序的扩展点；
     - 支持基础比较模式：字典序、数值升序/降序；与 `tva nl`、未来的 `tsv-select` 等保持一致的字段/列语法；
     - 对小文件使用内存内排序，对大文件采用“分块排序 + 临时文件归并”的模式，借鉴 `tsv-sort-fast` 与 `qsv sort` 的经验；
     - 在 CLI 设计上，优先向 GNU `sort` 的常用选项对齐（例如 `-k` 指定 key、`-n` 数值排序、`-r` 逆序），但在文档中明确与上游的差异。
   - 第二阶段目标：
     - 引入 header 识别与按列名排序能力，配合未来的字段语法模块；在 `tva` 的整体设计中，让 `sort`、`tsv-select`、`tsv-uniq` 等子命令共享同一套“列/字段选择”实现；
     - 在排序稳定性、缺失值处理（空字段）、本地化（例如大小写折叠、locale 排序）等方面做更多可配置的扩展；
