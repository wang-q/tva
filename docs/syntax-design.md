...
**Shell 友好 (Shell-friendly)**: 语法设计避免与 Shell 特殊字符冲突（如 `$`, `!`, `` ` ``），减少用户转义负担。
...
使用 `as` 关键字在管道中绑定变量，用于存储中间结果。

变量与列引用使用相同的 `@` 前缀，便于理解和记忆：
...
**说明**：
- `expr as @name` - 将表达式结果绑定到变量
- `expr as @name | ...` - 绑定后在后续管道中使用

**命名规则**：
- `@` + 小写字母开头，可包含字母、数字、下划线
- 可以遮蔽列引用 `@name`，但不会改变真实的列的值
- 对 TSV 的当前行生效，后续管道和语句中可以使用该变量；进入下一行时，列引用与变量值都会重置

**示例**：
```rust
// 计算含税总价
@price * @qty as @subtotal | @subtotal * (1 + @tax_rate)
...
**设计说明**：
- 采用 `as` ，与管道语义更一致（"将左侧结果命名为..."）
- 变量与列引用统一使用 `@` 前缀，降低认知负担
- 参考 jq 的 `expr as $var` 语法，但去掉 `$` 避免 Shell 冲突
...
**已完成任务**:
-   ✅ **变量绑定**: `expr as @name` 语法，支持在管道中绑定中间结果
-   ✅ **管道操作符**: `|` 支持函数链式调用，包括单参数和多参数（使用 `_` 占位符）
-   ✅ **字符串拼接**: `++` 操作符用于字符串连接
-   ✅ **注释支持**: `//` 行注释
-   ✅ **多表达式**: `;` 分隔多个表达式，支持副作用和变量绑定
-   ✅ **列表字面量**: `[1, 2, 3]` 语法
...
-   方法调用语法: `@name.trim()` 等价于 `trim(@name)`
...
变量绑定采用**行级作用域**：
- 变量在当前行内有效，进入下一行时自动清空
- 变量可以遮蔽同名列引用（如 `@price as @price`）
- 列引用 `@name` 优先检查变量，再回退到列名查找
...
管道操作符 `|` 的求值采用**立即求值**策略：
1. 求值左侧表达式得到值 `v`
2. 将 `v` 作为第一个参数传递给右侧函数
3. 对于多参数函数，使用 `_` 占位符显式指定参数位置
...
**示例**:
```rust
@price | int() as @p; @qty | int() as @q; @p * @q
// 步骤1: 将 @price 转为整数并绑定到 @p
// 步骤2: 将 @qty 转为整数并绑定到 @q
// 步骤3: 计算 @p * @q，返回结果
```
