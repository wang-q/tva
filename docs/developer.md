...
| 命令 | xan 对应 | 功能描述 | 价值 |
| :--- | :--- | :--- | :--- |
| **explode** | `explode` | 将包含分隔符的单元格拆分为多行 | 数据规范化常用操作，与 `longer` 类似但针对单元格 |
| **flatten** | `flatten` | 垂直展示记录，每行一个字段 | 便于查看宽表单行数据，类似 `view` 但更简单 |
...
| 命令 | xan 对应 | 功能描述 | 价值 |
| :--- | :--- | :--- | :--- |
| **cluster** | `cluster` | 文本聚类 | 数据探索 |
| **fuzzy_join** | `fuzzy_join` | 模糊匹配连接 | 处理脏数据 |
| **implode** | `implode` | `explode` 的逆操作 | 数据聚合 |
| **matrix** | `matrix` | 矩阵运算 | 科学计算场景 |
| **network** | `network` | 网络图分析 | 图数据场景 |
| **progress** | `progress` | 显示处理进度 | 大文件用户体验 |
| **range** | `range` | 生成数字序列 | 测试数据生成 |
| **search** | `search` | 多模式搜索（Aho-Corasick） | 高性能关键词搜索 |
| **separate** | `separate` | 使用正则拆分列 | 比 `unpack` 更灵活 |
| **tokenize** | `tokenize` | 文本分词 | NLP 预处理 |
| **vocab** | `vocab` | 词汇表统计 | NLP 场景 |
...
| `flatmap` | `flatmap` | 可用 `explode` 替代 |
| `input` | `input` | 功能简单，可用其他方式替代 |
| `map` | `map` | 需要表达式引擎 |
| `merge` | `merge` | 可用 `join` 替代 |
