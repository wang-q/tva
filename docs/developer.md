...
**已实现的 ggplot2 美学**:
*   ✅ `x` / `y`: 位置映射（必需）- `-x`, `-y` 参数
*   ✅ `colour` / `color`: 颜色映射 - `--color` 参数
*   ✅ `group`: 分组（通过 color 隐式实现）

**待实现的美学映射（按优先级）**:

*高优先级*:
*   **透明度 `alpha`**: 添加 `--alpha <0.0-1.0>` 参数控制整体透明度，或 `--alpha-col <col>` 按列映射透明度
*   **形状 `shape`**: 终端限制下，考虑用不同字符（`○□△◆`）或 Braille 密度表示
*   **大小 `size`**: 终端限制下，考虑用字符重复次数或密度表示

*中优先级*:
*   **统计变换 `stat`**: 添加 `--stat <smooth>` 支持回归线（`geom_smooth` 等价物）
*   **位置调整 `position`**: 添加 `--position <jitter>` 支持抖动（避免点重叠）

**非美学功能改进**:

*高优先级*:
*   **Marker 类型扩展**: 添加 `halfblock` 和 `bar` 标记类型（xan 支持 5 种：braille, dot, halfblock, bar, block）
*   **刻度格式化**: 根据数据范围自动选择精度（目前是固定 `{:.2}`，应利用已有的 `nice_midpoint` 逻辑）
*   **缺失值处理**: 添加 `--na.rm` 参数控制是否忽略含 NA 的行（默认 TRUE）

**坐标轴自动优化（无需新参数，全自动）**:
*   **刻度数量自适应**: 根据图表宽度/高度自动计算合适的刻度数量（如每 20 字符一个刻度），而非固定 3 个
*   **"Nice" 刻度算法**: 让刻度落在 1, 2, 5 的倍数上（类似 ggplot2 的 `pretty()`），避免出现 0.33 这类不友好的数值

*低优先级（命令行绘图以快速查看为主，避免过多参数）*:
*   ~~刻度控制~~: 不添加 `--x-breaks`/`--y-breaks`，保持自动计算
*   ~~刻度标签~~: 不添加 `--x-labels`/`--y-labels`，保持自动生成
*   ~~坐标轴变换~~: 暂不考虑 `--x-trans`/`--y-trans`，如有需要可后续添加

*中优先级*:
*   **回归线**: 添加 `--smooth` 或 `--regression` 参数叠加回归线
*   **置信区间**: 回归线时显示置信区间阴影

**关于 `--line` 参数的改进（参考 ggplot2 `geom_path` vs `geom_line`）**:

目前 `--line` 的实现类似于 `geom_line()`：自动按 x 值排序后连接点。但 ggplot2 区分了两种几何对象：

*   **`geom_path()`**: 按数据原始顺序连接点，适用于时间序列、轨迹等有序数据
*   **`geom_line()`**: 按 x 值排序后连接，适用于函数图像、趋势图

**改进建议**:
*   ✅ `--line` 保持现状（按 x 排序）- 已实现
*   ✅ `--path` 参数，按数据原始顺序连接（不排序）- 已实现
