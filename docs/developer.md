...
```bash
git log v0.2.2..HEAD > gitlog.txt
git diff v0.2.2 HEAD -- "*.rs" "*.md" > gitdiff.txt
...
XXX 的测试覆盖度很低，使用 `cargo llvm-cov` 生成覆盖率报告，找出需要提升的地方.
...
> 用户文档见 `docs/help/from_html.md`、`docs/from.md` 和 `docs/selectors.md`。本节仅记录实现相关的技术细节。

### 三种模式的核心函数

| 模式 | 触发条件 | 核心函数 | 输出格式 |
| :--- | :--- | :--- | :--- |
| **Query Mode** | `--query` / `-q` | `extract_query()` | HTML/Text |
| **Table Mode** | `--table` 或 `--index` | `extract_table()` | TSV |
| **Struct Mode** | `--row` + `--col` | `extract_struct()` | TSV |

参数互斥关系由 `clap` 的 `conflicts_with_all` 和 `requires` 保证。

### 关键技术点

**Table Mode:**
*   遍历 `thead`/`tbody`/`tfoot` 的直接子元素，避免嵌套表格污染
*   使用 `csv::Writer` 输出 TSV

**Struct Mode:**
*   `--col` 格式解析：`Name:Selector Func`
*   支持两种函数风格：`text{}`/`attr{name}`（pup 风格）和 `text()`/`attr("name")`（传统风格）
*   默认函数为 `Text`

**输入输出:**
*   输入：`raw_reader()` 支持文件/stdin，完整读取后使用 `scraper::Html::parse_document()` 解析
*   输出：`writer()` 支持文件/stdout

### 未实现特性

*   `pup` 的 `json{}` 输出格式
*   `pup` 特有伪类（如 `:parent-of`）
