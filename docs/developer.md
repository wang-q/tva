...
**功能定位**:
基于 `tva stats` 已实现的统计量（min, max, median, q1, q3, mad），绘制箱线图展示数据分布。

**CLI 设计**:
```bash
# 基本用法 - 绘制单列箱线图
tva plot box data.tsv -c age

# 按组分别绘制
tva plot box data.tsv -c age --color label

# 多列对比
tva plot box data.tsv -c age,height,weight

# 指定图表尺寸
tva plot box data.tsv -c age --cols 80 --rows 30
```

**统计量复用**:
箱线图需要的统计量已全部在 `tva stats` 中实现：
*   **Min/Max**:  whisker 边界（或直接用 min/max）
*   **Q1 (25%)**:  箱体下边界 - `stats` 已实现
*   **Median (50%)**:  箱体中线 - `stats` 已实现  
*   **Q3 (75%)**:  箱体上边界 - `stats` 已实现
*   **MAD**:  可用于计算异常值边界 - `stats` 已实现

**渲染设计**:
*   使用 `ratatui` 的 `BarChart` 或自定义 `Canvas` 绘制
*   箱体用 `█` 或 `▓` 填充，whisker 用 `│` 或 `─`
*   异常值用 `•` 或 `○` 单独标记
*   多组数据水平排列，每组一个箱线图

**实现步骤**:
1.  复用 `stats` 的聚合引擎计算各组统计量
2.  使用 `libs/plot/chart.rs` 的渲染基础设施
3.  设计箱线图的专用渲染函数（类似 `render_chart`）
4.  添加颜色支持（多组对比时）
5.  编写测试和文档

**参考实现**:
*   `ggplot2::geom_boxplot()` - 箱线图美学生态
*   `xan plot` - 终端图表渲染技巧
