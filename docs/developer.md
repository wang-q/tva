...
*   **与 xan plot 的差异**:
    ...
    *   TSV 优先：利用 TSV 无引号特性，使用 `fast_float` 进行快速数值解析
...
2.  **数据采集**:
    *   流式读取 TSV，使用 `fast_float::parse` 解析数值
...
*   **依赖选择**:
    ...
    *   `fast_float`: 快速浮点数解析（比标准库快 2-3 倍）
    *   `crossterm`: 终端尺寸检测和颜色支持

*   **性能优化**:
    *   零拷贝解析：TSV 字段直接切片，避免字符串分配
    *   预分配 Vec：根据文件大小预估点数，减少 realloc
    *   增量渲染：大数据集时，可考虑采样或分块渲染
...
*   **颜色方案**：调色板选择（8色/16色/256色）、颜色循环策略
*   **坐标轴标签**：是否显示列名作为轴标签、是否支持 `--title` 参数
*   **折线连接顺序**：按文件行顺序还是按 X 值排序后连接
*   **大数据集阈值**：触发采样的点数阈值、采样算法选择
*   **测试策略**：图形输出的测试方案（snapshot 测试或数据结构验证）
