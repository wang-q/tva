...
```bash
git log v0.2.1..HEAD > gitlog.txt
git diff v0.2.1 HEAD -- "*.rs" "*.md" > gitdiff.txt
...
1.  **P0**: `fill` (仅 Down 模式) - 实现简单，价值高，完全流式。
2.  **P1**: `longer` - 补全重塑功能，完全流式。
3.  **P1**: `separate` / `unite` - 数据清洗刚需，完全流式。
4.  **P2**: `fill` (Up 模式) - 需多次 IO 或内存缓冲。
