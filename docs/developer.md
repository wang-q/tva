...
`plot point` 和 `plot box` 已实现。实现细节：
*   **Liang-Barsky 裁剪**: ✅ 已实现 `clip` 算法，用于计算回归线在图表边界内的端点。
*   **箱线图**: ✅ 已实现 `tva plot box` 命令，支持分组、多列、异常值显示。
...
### 6. tva plot bin2d 设计分析

`ggplot2::geom_bin_2d()` 将平面划分为矩形网格，统计每个网格内的点数，用颜色深浅表示密度。这与 `geom_tile()` 的关系类似于 `bin` 与原始数据的关系。

#### 与现有命令的关系

| 命令组合 | 功能 | 局限性 |
| :--- | :--- | :--- |
| `tva bin \| tva bin` | 两次一维分箱 | 丢失组合信息，无法直接计数 |
| `tva bin \| tva stats -g` | 分箱后统计 | 需要三次命令，输出格式需额外处理 |
| `tva plot bin2d` | 自动二维分箱 + 可视化 | 探索阶段使用，不输出原始数据 |

#### 设计建议

**工作流程**: `plot bin2d` (探索) -> `bin` (精确处理)

```bash
# 1. 快速探索 - 自动分箱看图
# 自动选择分箱参数，用热力图展示数据密度分布
tva plot bin2d data.tsv -x age -y income

# 2. 根据图形确定最佳参数，进行精确处理
# 观察到模式后，用确定的参数进行生产级处理
...
1. **不复用 `bin` 命令**: `tva bin` 是流式 ETL 工具，输出逐行数据；`plot bin2d` 需要聚合统计，两者用途不同。

2. **内部复用分箱算法**: `plot bin2d` 可以复用 `bin` 的核心分箱逻辑，但增加计数和渲染功能。

3. **自动分箱参数**: 参考 `xan bins` 的启发式算法（Freedman-Diaconis 等），自动选择合理的分箱数。

4. **可视化友好**: 使用 `ratatui` 渲染热力图，字符密度（`░▒▓█`）或颜色表示计数。
