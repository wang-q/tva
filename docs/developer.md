...
### 3. tva plot point 命令设计计划

基于对 `xan plot` 的深入分析，计划为 `tva` 实现一个简化但功能完整的 `plot point` 命令，专注于散点图和折线图的终端可视化。

#### 3.1 核心定位

*   **命令名**: `tva plot point`
*   **目标**: 提供轻量级的终端散点图/折线图绘制能力
*   **与 xan plot 的差异**:
    *   简化架构：不依赖复杂的 SeriesBuilder，直接基于列索引选择数据
    *   TSV 优先：利用 TSV 无引号特性，使用 `lexical` 进行快速数值解析
    *   适度功能：支持散点图、折线图、分类着色，暂不支持 Small Multiples 和回归线
    *   终端友好：使用 `TestBackend` 避免输出重叠，同时保留颜色支持
*   **表头假设**：默认输入 TSV 包含表头，使用列名或 1-based 索引引用列，不提供 `--header` 参数

#### 3.2 CLI 设计（参考 ggplot2 geom_point）

遵循 ggplot2 的**美学映射（Aesthetic Mappings）**理念，将数据列映射到视觉属性：

**必需参数（Required Aesthetics）**:
*   `-x, --x <col>`: X 轴列（1-based 索引或列名），对应 `aes(x = ...)`
*   `-y, --y <col>`: Y 轴列，对应 `aes(y = ...)`

**可选美学映射（Optional Aesthetics）**:
*   `--color <col>`: 颜色映射列，对应 `aes(color = ...)`，按该列的值分组着色
*   `--size <col>`: 大小映射列，对应 `aes(size = ...)`，暂不支持（终端限制）
*   `--shape <col>`: 形状映射列，对应 `aes(shape = ...)`，暂不支持

**几何类型（Geometry）**:
*   `-l, --line`: 绘制折线图（默认散点图），对应 `geom_line()` vs `geom_point()`

**图表属性（Plot Attributes）**:
*   `--cols <n>`: 图表宽度（字符数或比例如 "0.8"）
*   `--rows <n>`: 图表高度（字符数或比例如 "0.6"）
*   `-m, --marker <type>`: 标记类型：`braille`（默认）、`dot`、`block`
*   `--xlim <min,max>`: X 轴范围限制
*   `--ylim <min,max>`: Y 轴范围限制

**输入/输出**:
*   `[infile]`: 输入 TSV 文件（默认 stdin）
*   `-o, --outfile <file>`: 输出文件（默认 stdout，纯文本图表）

#### 3.3 处理流程

1.  **参数解析与美学映射**:
    *   解析必需美学（x, y）和可选美学（color）
    *   验证列名/索引存在且可解析为数值

2.  **数据采集**:
    *   流式读取 TSV，使用 `crate::libs::number::fast_parse_f64()` 解析数值
    *   根据 `--color` 列值分组存储：`IndexMap<String, Vec<(f64, f64)>>`
    *   无 `--color` 时，所有点放入默认组 ""

3.  **数据处理**:
    *   域计算：遍历所有点，计算 x/y 的 min/max（考虑 `--xlim/--ylim`）
    *   坐标轴刻度：基于终端尺寸自动计算合理刻度

4.  **渲染**:
    *   使用 `ratatui` 的 `Chart` widget
    *   每个 color 组作为一个 `Dataset`，使用不同颜色
    *   标记：`Braille`（高密度）、`Dot`、`Block`
    *   图例：当 `--color` 启用时，显示颜色图例

#### 3.4 关键技术决策（已实现）

*   **依赖选择**:
    *   `ratatui`: 终端 UI 框架，提供 Chart widget
    *   `lexical`: 通过 `crate::libs::number::fast_parse_f64()` 进行快速浮点数解析
    *   `colored`: 终端颜色输出支持
    *   `crossterm`: 终端尺寸检测

*   **渲染策略**:
    *   使用 `TestBackend` 而非 `CrosstermBackend` 渲染图表，避免与终端现有内容重叠
    *   通过 `print_buffer_to_stdout()` 手动打印缓冲区内容，同时保留颜色信息
    *   使用 `group_cells_by_color()` 将相同颜色的单元格分组，实现高效的彩色输出

*   **性能优化**:
    *   零拷贝解析：TSV 字段直接切片，避免字符串分配
    *   预分配 Vec：使用 `IndexMap` 存储分组数据
    *   颜色分组渲染：将相同颜色的字符批量输出，减少 ANSI 转义序列切换

*   **错误处理**:
    *   `--ignore`: 跳过无法解析的数值行
    *   空数据检测：如果解析后无有效点，返回友好错误

#### 3.5 实现细节（已确定）

*   **颜色方案**：使用 6 色循环调色板（Cyan, Red, Green, Yellow, Magenta, Blue），通过 `COLORS` 常量定义
*   **坐标轴标签**：自动使用列名作为轴标签（X 轴标签在底部，Y 轴标签在顶部）
*   **折线连接顺序**：使用 `--line` 时，按 X 值排序后连接（`sorted_points.sort_by(|a, b| a.0.partial_cmp(&b.0).unwrap())`）
*   **终端尺寸**：默认使用终端尺寸的 70% 宽度和 50% 高度，可通过 `--cols` 和 `--rows` 覆盖
*   **测试策略**：使用 `assert_cmd` 进行集成测试，验证命令行参数解析、错误处理和输出格式

#### 3.6 使用示例

```bash
# 基础散点图（类似 ggplot2: ggplot(data, aes(x=col1, y=col2)) + geom_point()）
tva plot point -x 1 -y 2 data.tsv

# 折线图（类似 ggplot2: ... + geom_line()）
tva plot point -l -x 1 -y 2 data.tsv

# 按类别着色（类似 ggplot2: ... + aes(color=species)）
tva plot point -x sepal_width -y sepal_length --color species iris.tsv

# 指定图表尺寸和范围
tva plot point -x 1 -y 2 --cols 100 --rows 30 --xlim 0,10 --ylim -5,5 data.tsv
```

#### 3.7 未来扩展

*   **回归线**: 计算线性回归并在散点图上叠加
*   **Small Multiples**: 支持 `-S` 参数分面展示
