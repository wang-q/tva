...
fn expr_global_var_index() {
    let (stdout, _) = TvaCmd::new()
        .args(&[
            "expr", "-n", "name", "-r", "alice", "-r", "bob", "-r", "charlie", "-E",
            "@__index",
        ])
        .run();
    ...
    assert!(
        lines[0].contains("1"),
        "Expected '1' in first line, got: {}",
        lines[0]
    );
    assert!(
        lines[1].contains("2"),
        "Expected '2' in second line, got: {}",
        lines[1]
    );
    assert!(
        lines[2].contains("3"),
        "Expected '3' in third line, got: {}",
        lines[2]
    );
}
...
fn expr_global_var_accumulator() {
    ...
    assert!(
        lines[0].contains("10"),
        "Expected '10' in first line, got: {}",
        lines[0]
    );
    assert!(
        lines[1].contains("30"),
        "Expected '30' in second line, got: {}",
        lines[1]
    );
    assert!(
        lines[2].contains("60"),
        "Expected '60' in third line, got: {}",
        lines[2]
    );
}
...
fn expr_global_var_persists_across_rows() {
    ...
    assert!(
        lines[1].contains("3"),
        "Expected '3' in second line, got: {}",
        lines[1]
    );
    assert!(
        lines[2].contains("6"),
        "Expected '6' in third line, got: {}",
        lines[2]
    );
}
...
fn expr_global_var_string_concat() {
    ...
    assert!(
        lines[1].contains("alice,bob,"),
        "Expected 'alice,bob,' in second line, got: {}",
        lines[1]
    );
    assert!(
        lines[2].contains("alice,bob,charlie,"),
        "Expected 'alice,bob,charlie,' in third line, got: {}",
        lines[2]
    );
}
...
fn expr_global_var_read_and_write() {
    ...
    assert!(
        lines[0].contains("5"),
        "Expected '5' in first line, got: {}",
        lines[0]
    );
    assert!(
        lines[1].contains("15"),
        "Expected '15' in second line, got: {}",
        lines[1]
    );
    assert!(
        lines[2].contains("30"),
        "Expected '30' in third line, got: {}",
        lines[2]
    );
}
