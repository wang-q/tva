...
以下是 AI 助手在开发过程中总结的核心经验与规则，请严格遵守：

1.  **防止代码误删 (Anti-Deletion in Repetitive Files)**:
    *   在修改具有重复模式的文件（特别是测试文件 `tests/*.rs`）时，**必须**确保 `SearchReplace` 工具的 `old_str` 包含足够的**唯一上下文**（如函数名、特定的注释或唯一的代码行）。
    *   如果目的是在文件末尾追加代码，**必须**先读取文件的末尾部分，并确保 `old_str` 准确匹配文件末尾的闭合结构（如 `mod tests { ... }` 的最后一个 `}`），或者使用更明确的定位点，避免意外覆盖中间的代码块。
